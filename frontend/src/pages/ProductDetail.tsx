...
const ProductDetail: React.FC = () => {
  ...
  <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-start gap-10 px-8 py-12 animate-fadeIn">
    ...
    <div className="flex flex-col justify-start space-y-4 text-left w-full max-w-xl">
      ...
      <div className="space-y-1 text-sm text-muted-foreground">
        <p><span className="font-semibold text-foreground">Starring:</span> {movie.cast}</p>
        <p><span className="font-semibold text-foreground">Director:</span> {movie.director}</p>
        <p><span className="font-semibold text-foreground">Genre:</span> {genreList || "Unknown"}</p>
      </div>
  ...
  <div className="space-y-8 max-w-7xl mx-auto px-8 pb-12">
    {["Recommended for you", "User Reviews", "Trailers & Behind the Scenes"].map(
      (section, index) => (
        <div
          key={index}
